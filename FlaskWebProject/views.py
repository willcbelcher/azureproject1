...
def post(id):
    ...
    if form.validate_on_submit():
        if form.submit.data:
            ...
            post.image_path = ""
            return redirect(url_for('/post/' + str(id)))
    return render_template(
        'post.html',
        title='Edit Post',
        imageSource=imageSourceUrl,
...
def _build_msal_app(cache=None, authority=None):
    return msal.ConfidentialClientApplication(Config.CLIENT_ID, client_credential = Config.CLIENT_SECRET, authority = authority or Config.AUTHORITY,token_cache = cache)
